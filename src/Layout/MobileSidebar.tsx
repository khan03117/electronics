...
const MobileSidebar: React.FC<Prop> = ({ open, setOpen }) => {
    ...
    const loadMore = () => {
        setStep(prevCount => prevCount + 6); // Load 10 more items
    };
    ...
    const allProducts = catproducts.flatMap(pdt => pdt.category.products);
                    ...
                    <div className="col-span-3">
                        <PerfectScrollbar className='max-h-[85vh] overflow-x-hidden'>
                            ...
                            <div className="w-full h-full flex flex-col">
                                ...
                                {category.map(cat => (
                                    <>
                                        <button onClick={() => handleScat(cat.url)} className={`w-full text-xs text-start border px-2 border-blue-gray-200 py-6 ${scat == cat.url ? 'bg-white border-0 border-l-4 border-primary' : 'bg-[#ecedf1] '}`}>
                            ...
                            <div className="w-full">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    ...
                                    {visibleProducts.length < allProducts.length && (
                                        <div className="col-span-3 text-center block mt-4">
                                            <button
                                                onClick={loadMore}
                                                className="px-4 py-2 bg-primary inline-flex mx-auto items-center gap-2 uppercase text-sm rounded-full text-white "
                                            >
                                                Load More <span className="inline"><IoChevronDownSharp /></span>
                                            </button>
                                        </div>
                                    )}
                                </div>
